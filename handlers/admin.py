...
async def excel_import_upload(message: Message):
    ...
    # Attestatsiya savollari import qilingan bo'lsa — JSON yangilash
    if added > 0:
        try:
            from handlers.question_editor import generate_bolim_json
            for bn in range(1, 11):
                await generate_bolim_json(bn)
            await message.answer("🔄 Attestatsiya JSON fayllar yangilandi.")
        except Exception as e:
            pass
...
async def regenerate_all_json(message: Message):
    ...
    await message.answer(
        ...
        f"GitHub Pages da 1-2 daqiqa ichida ko'rinadi."
    )