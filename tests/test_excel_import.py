...
async def parse_xlsx(buf: io.BytesIO) -> tuple:
    """Excel faylini parse qilib savollarni DB ga saqlaydi."""
...
class TestExcelImport:
    ...
    async def test_valid_choice_questions(self, test_db):
        ...
        added, skipped, errors = await parse_xlsx(make_xlsx(rows))
        assert added == 2
        assert skipped == 0
        assert errors == []
    ...
    async def test_mixed_valid_invalid(self, test_db):
        ...
        rows = [
            ...
            ["noto_g_ri", "aralash", None, "FALSE", None,
